...
def _load_all():
    ...
    return crsp, cons, delist, ibes, rs
...
def _run(crsp, cons, delist, ibes, rs, **kw):
    return run_backtest(
        ...
        formation_years=2, metric="ssd", verbose=False, **kw
    )
...
def fig_regime_calendar(rs: pd.Series, crsp, cons, delist, ibes, out: Path):
    ...
    # Classify each month: majority HMM state
    hmm = rs.reindex(pd.date_range("2016-01-01", "2023-12-31", freq="D"), method="ffill").dropna()

    def monthly_regime(month_end):
        mo = pd.Period(month_end, "M")
        subset = hmm[(hmm.index.to_period("M") == mo)]
        if len(subset) == 0:
            return "calm"
        avg = subset.mean()
        if avg >= 0.75:
...
def main():
    ...
    crsp, cons, delist, ibes, rs = _load_all()
    ...
    if "calendar" not in args.skip:
        print("\n[2/5] Regime calendar...")
        fig_regime_calendar(rs, crsp, cons, delist, ibes, out)

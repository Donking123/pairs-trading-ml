...
def rolling_zscore(
    ...
    (never day t — enforced via `.shift(1)`). σ is either:
      ...
      - a fixed `frozen_sigma` estimated on the formation window (deepak-strategy
        improvement: anchors volatility to what was observed before trading began,
        so a spread that widens mid-trading-month doesn't inflate the denominator
        and make z-scores artificially small).
    ...
    frozen_sigma : float or None
        ...
        Pass `HedgeRatio.residual_std` (formation-window OLS spread σ).

    Returns
    -------
    Series
        z-scores; first `window` entries are NaN when frozen_sigma is None
        (rolling std needs history); all entries defined when frozen_sigma set.
    """
    if window < 2:
